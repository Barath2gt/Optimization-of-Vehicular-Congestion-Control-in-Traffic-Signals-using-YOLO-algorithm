...
directionNumbers = {1:'right', 2:'down', 3:'left', 0:'up'}


signalCoods = [(510,220),(830,220),(830,570),(510,570)]
signalTimerCoods = [(510,180),(830,180),(830,540),(510,540)]
vehicleCountCoods = [(480,180),(880,180),(880,540),(480,540)]
...
def calculateWaitingTime():
    ...
    for direction in vehicles:
        ...
        for lane in range(noOfLanes):
            total_waiting_time = sum([vehicle.waiting_time for vehicle in vehicles[direction][lane]])
            waiting_times[direction][lane] = total_waiting_time
...
class Main:
    ...
    pygame.display.set_caption("TRAFFIC OPTIMIZATION")

    directionImage = pygame.image.load('images/direction.png')
    ...
    while True:
        ...
        screen.blit(directionImage,(1300,10))
        ...
        # display signal timer and vehicle count
            # signalTexts[i] = font.render(str(signals[i].signalText), True, (255, 0, 0),black)
            # screen.blit(signalTexts[i],signalTimerCoods[i]) 
            # displayText = vehicles[directionNumbers[i]]['crossed']
            # # vehicleCountTexts[i] = font.render("Vehicles passed: " + str(displayText), True,black,white)
            # vehicleCountTexts[i] = font.render(str(displayText), True,black,white)
            # screen.blit(vehicleCountTexts[i],vehicleCountCoods[i])

        for i in range(0,noOfSignals):  
            if(str(signals[i].signalText) =="Stop"):
                signalTexts[i] = font.render(str(signals[i].signalText), True, (255, 0, 0),black)
            elif(str(signals[i].signalText) =="Wait"):
                signalTexts[i] = font.render(str(signals[i].signalText), True, (255, 255, 0),black)
            elif(str(signals[i].signalText) =="Go"):
                signalTexts[i] = font.render(str(signals[i].signalText), True, (0, 255, 0),black)
            else:
        ...
        screen.blit(timeElapsedText,(800,50))
